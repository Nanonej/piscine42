...
/*   Updated: 2016/09/18 18:54:45 by rmonnier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft.h"

char	ft_def_char_by_pos3(int x, int y, int column, int line)
{
	if (column == 1 && line == 1)
		return ('A');
...
char	*colle03(int y, int x)
{
	...
	if (x > 0 && y > 0)
	{
		...
		while (line <= y)
		{
			...
			while (column <= x)
			{
				tab[pos++] = ft_def_char_by_pos3(x, y, column, line);
				column++;
			}
			tab[pos++] = '\n';
			line++;
